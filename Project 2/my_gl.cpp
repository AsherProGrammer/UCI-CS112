...
// Includes declaration of I_my_gl functions.
#include "i_my_gl.h"
#include<iostream>
// The followings are wrapper functions.

// Synchronizes OpenGL's current matrix with i_my_gl's current matrix.
void sync_matrix()
{
    GLdouble m[16];
    I_my_glGetMatrixd(m);
...
// Switches matrix mode for both OpenGL and i_my_gl. 
...
bool compareMat(GLdouble* left, GLdouble* right)
{
	//0.0001 is approx. for Machine epsilon
	for (GLint i = 0; i < 16; i++)
	{
		if (abs(left[i] - right[i]) > 0.0001)
		{
			return false;
		}
	}
...
void printMat(GLdouble* matrix)
{
	for (GLint i = 0; i < 4; i++) {
		printf("%f %f %f %f\n", matrix[4 * 0 + i], matrix[4 * 1 + i], matrix[4 * 2 + i], matrix[4 * 3 + i]);
	}
	printf("\n");
}