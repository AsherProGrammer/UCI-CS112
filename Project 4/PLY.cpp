...
void PLYObject::draw()
{
	...
	else
	{
		...
		Vector3f V, L, H, amb, dif, spe, total = {0,0,0}, View,Light, newVertice, newNormal;
		Vector4f emission;
		GLfloat att, dis;
		float M[16];
		Matrix4f m;
		...
		for (int v = 0; v<nv; v++)
		{
			...
			multVector(Light, m, light_pos);
			multVector(View, m, viewer_pos);
			multVector(newVertice, m, vertices[v]);
			multVector(newNormal, m, normals[v]);
			
			//half vector
			//sub(L, Light, newVertice);
			for (GLint i = 0; i < 3; i++) {
				L[0] = Light[0];
				L[1] = Light[1];
				L[2] = Light[2];
			...
			//sub(V, View, newVertice);
			for (GLint i = 0; i < 3; i++) {
				V[0] = View[0];
				V[1] = View[1];
				V[2] = View[2];
			}
			//sub(V, viewer_pos, vertices[v]);
			normalize(L);
			normalize(V);
			add(H, L, V);
			dis = normalize(H);
			...
			//model ambient
			Vector4f modelAmbientLight;
			Vector3f modelAmb;
			glGetFloatv(GL_LIGHT_MODEL_AMBIENT, modelAmbientLight);
			multVectors(modelAmb, modelAmbientLight, ambient);

			//ambient
			multVectors(amb, ambient, ambient_light);

			//diffuse
			multVectors(dif, diffuse, light_color);
			GLfloat Ln = dotProd(L, newNormal);
			//GLfloat Ln = dotProd(L, normals[v]);
			scale(dif, max(Ln, 0));

			//specular
			if (Ln > 0) {
				GLfloat s = dotProd(H, newNormal);
				//GLfloat s = dotProd(H, normals[v]);
				multVectors(spe, specular, light_color);
				scale(spe, pow(max(s, 0), shininess[0]));
			}
			else {
				scale(spe, 0);
			}
			...
			att = 1 / (constantAttenuation + linearAttenuation*dis + quadraticAttenuation*dis*dis);
			...
			//color
			colors[v][0] = (amb[0] + dif[0] + spe[0]) * 255;
			colors[v][1] = (amb[1] + dif[1] + spe[1]) * 255;
			colors[v][2] = (amb[2] + dif[2] + spe[2]) * 255;
